...
use Bueno\Validations\CreateSocalUserValidator;
...
class AuthController extends Controller
{
  ...
  /**
   * @var CreateSocalUserValidator
   */
  protected  $socailValidator;
   ...
   * @return void
   */
  public function __construct(DbCommonRepository $commonRepo, AuthenticateUser $authenticate, CreateUserValidator $validator, CreateSocalUserValidator $socailValidator, DbUserRepository $userRepo, ResetPasswordValidator $resetPasswordValidator)
  {
    $this->userRepo = $userRepo;

    $this->validator = $validator;

    $this->socailValidator = $socailValidator;
  ...
  public function socialRegister()
  {
    ...
    return view('users.socailregister', compact('page'));
  }
   ...
   * @return $this|\Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector
   */
  public function postSocailRegister()
  {
    $inputs = request()->all();

    try
    {
      $this->socailValidator->fire($inputs);
    }
    catch(ValidationException $e)
    {
      return redirect(route('users.socialregister.get'))->withInput()->withErrors($e->getErrors());
    }
    $inputs['password'] = '';
    $user = $this->authenticate->registerUser($inputs);
