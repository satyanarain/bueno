...
class CheckoutController extends Controller
{
  ...
  function __construct(Billing $billing, OrderRepo $orderRepo, UserRepo $userRepo, CommonRepo $commonRepo, KitchenRepo $kitchenRepo, Validator $validator)
  {
    ...
    $this->middleware('auth', ['except' => ['olamoneyNotify']]);
  }
  ...
  {
    $inputs = request()->all();
    $inputs['address_id'] = !empty($inputs['address_id']) ? $inputs['address_id'] : config('bueno.pickup_default_address');
    try
    {
  ...
  public function applyCoupon()
  {
    ...
    /**
   * handles the notification response from payment gateway
   *
   * @param $payment_mode_id
   */

  public function olamoneyNotify($payment_mode_id)
  {
    $inputs = request()->all();

    $payment = $this->billing->resolve();

    return $payment->handleNotify($inputs);
  }
