...
use Bueno\Transformers\KitchenTransformer;
...
class ApiController extends Controller
{
  ...
  function __construct(
        ...
        KitchenTransformer $kitchenTransformer,
      ...
      )
  {
    ...
    $this->kitchenTransformer =  $kitchenTransformer;
  ...
  public function getService()
  {
    ...
    switch($inputs['service_type'])
    {
      ...
      case 'get_area':
        $data = $this->getUserArea($inputs);
        break;

      case 'get_area_kitchen':
        $data = $this->getAreaKitchen($inputs);
        break;

      case 'get_order_by_id':
        $data = $this->getOrderById($inputs);
        break;
   ...
   * @return array
   */
  public function getUserArea($inputs)
  {
    $area = $this->locationRepo->getAreaById($inputs['id']);

    $area = $this->locationTransformer->transformApiLocality($area);
   ...
   * @return array
   */
  public function getAreaKitchen($inputs)
  {
    $area = $this->locationRepo->getAreaById($inputs['id']);

    $kitchen = $this->kitchenRepo->getAreaKitchen($inputs['id']);

    $kitchen = $this->kitchenTransformer->transform($kitchen);
    
    if(!$kitchen['delivery_charge']){
      $kitchen['delivery_charge'] = $area->delivery_charge;
    }
   ...
   * @return array
   */
  public function getOrderById($inputs)
  {
    $order = $this->orderRepo->getOrderById($inputs['id']);
   ...
   *
   * @return \Illuminate\Http\JsonResponse
   * Sorry, kitchen is closed at this time. Please come back at 8 AM.
   */
  public function getData()
  {
    $isKitchenOpen = $this->commonRepo->isKitchenOpen() ? 'false' : 'true';
    $data = '{
      "is_ordering_closed": '.$isKitchenOpen.',
      "ordering_closed_message": "Sorry, kitchen is closed at this time. Please come back at 8 AM.",
      ...
      "max_order_items": 10,
      "min_android_version": 1020160801,
      "max_credit_usage_percent": '.config('bueno.site.max_credit_usage_percent').',
      "kitchen_locations": [
        {
        "latitude": 28.460208,
        "longitude": 77.05506
      ...
      "no_dishes_error_message": "Sorry, kitchen is closed at this time. Please come back at 8 AM.",
      "payment_methods": [
        {
        "EBS": "Online payment"
        },
      ...
      "payment_methods_android": [
        {
        "Razorpay": "Credit Card, Debit Card, NetBanking, Wallets"
        },
        {
      ...
      $dataObj->ngos = $this->transformNgos($ngos);
   ...
   * @return array
   */
  private function transformNgos($ngos)
  {
    $data = [];
    ...
    foreach($ngos as $ngo)
    {
      $data[] = [
        ...
        'default_donation_amount' => $ngo->default_donation_amount,
        'display_image_url' => route('photo.ngos', $ngo->display_image_url),
      ];
    }
