...
class CartController extends Controller
{
  ...
  public function getCart()
  {
    ...
    $outofstock_items = $this->kitchenRepo->getOutOfStockItems(auth()->user()->cartItems, session('area_id'));
...
//      $this->kitchenRepo->checkIfItemsAreInStock(auth()->user()->cartItems);
    ...
    if(count($outofstock_items))
    {

      $this->userRepo->deleteOutOfStockCartItem(auth()->user(), $outofstock_items);      

      $cart_items = $this->userRepo->getAllCartItems(auth()->user()->id);
      
      auth()->user()->cartItems = $cart_items;

      Flash::info('Cart updated as some items are out of stock');
    }
    return view('users.cart', compact('cart_items','area', 'kitchen', 'ngos', 'session_area', 'checkout_errors', 'payment_modes', 'invoice_taxes'));
  }
