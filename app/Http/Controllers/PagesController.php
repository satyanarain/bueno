...
class PagesController extends Controller {
  ...
  {
    $seo = $this->commonRepo->getSeoBySlug('franchise');

    if($seo) $this->setSeo($seo->meta_title, $seo->meta_description,  $seo->image_url);

    $trending = $this->itemRepo->getTrendingItems(8);

    return view('pages.franchise', compact('trending'));
  }

  public function getDiksha()
  {
    $seo = $this->commonRepo->getSeoBySlug('diksha');

    if($seo) $this->setSeo($seo->meta_title, $seo->meta_description,  $seo->image_url);

    $trending = $this->itemRepo->getTrendingItems(8);

    return view('pages.diksha', compact('trending'));
  }
  ...
  public function getFranchiseDownload()
  {
    return response()->download(public_path() . '/downloads/franchise.pdf');
  }
  ...
  public function postFranchiseQuery()
  {
    $inputs = request()->all();
    try
    {
    ...
    {
      return redirect(route('pages.franchise.get'))->withErrors($e->getErrors())->withInput();
    }
    ...
    return redirect(route('pages.franchise.get'));
  }
