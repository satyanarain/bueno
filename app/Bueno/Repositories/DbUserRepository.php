...
class DbUserRepository  {
  ...
  public function deleteOutOfStockCartItem($user, $cart_ids)
  {
    foreach ($cart_ids as $cart_id) {

      $cart = Cart::where('user_id', $user->id)->where('item_id', $cart_id)->first();
  ...
  public function checkIfAddressExists($inputs, $user)
  {
    ...
    $address = Address::where('id', $inputs['address_id'])->first();

    if(!$address || (!in_array($address->user_id,[$user->id,0])))
    {
      throw new CheckoutValidationException('Sorry. The address selected is invalid');
    }
  }
