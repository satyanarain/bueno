    ...
    foreach($checks as $check)
    {
      ...
      $data['max_cashback_amount'] = 0;
          ...
          case 'free_item':
            ...
            {
              $data['success'] = 0;
              $data['message'] = 'Freebie item '.$item->itemable->name.' is out of stock.';
              $data['discount_applicable_on'] = 0;
              $data['max_discount_amount'] = 0;
              $data['max_cashback_amount'] = 0;
              return $data;
...
//Create a new Order - Back End Quick Order
  ...
  public function newOrder($user_id,$address_id,$request)
  {
    ...
    if($redeem_points==1)
    {
      ...
      if($user_points>0)
      {
        $max_percent = config('bueno.site.max_credit_usage_percent');
        if($user_points>=($amount * $max_percent /100))
        {
          $user_points_used = $amount * $max_percent /100;
        }
        else{
    ...
    $delivery_charge = $new_order->kitchen->delivery_charge ? $new_order->kitchen->delivery_charge : $new_order->area->delivery_charge;
    ...
    $service_charge_amount = $amount * $service_charge * 0.01;

    $total_tax =  $service_tax_amount + $vat_amount + $delivery_charge + $service_tax_amount + $service_tax_amount + $packaging_charge;
   ...
   // Service Charge
    ...
    $this->newOrderInvoice($new_order->id,'Service Charge',$service_charge_amount);
  ...
  public function rollbackOrderRedeemPoints($order)
  {
    if($order->redeem_points==1)
    {
      $credits_used =  $order->invoice->where('charge_for','Points Redeemed')->first();

      $credits_used = $credits_used ? (int) $credits_used->amount : 0;

      $user = User::find($order->user_id);

      $user->points += $credits_used;
  ...
  public function createUserOrder($inputs)
  {
    ...
    $order = Order::create([
      ...
      'is_pickup' => isset($inputs['is_pickup']) ? $inputs['is_pickup']: 0,
      'delivery_address'  => !isset($inputs['is_pickup']) ? $address->address . ', ' . $area->name . ', -' . $address->pincode: '',
  ...
  public function   createOrderApi($inputs)
  {
    ...
    if($inputs['redeem_points'] > ceil($order_amount_base * $max_percent / 100)) throw new CheckoutValidationException('You can only redeem '.$max_percent.'% of order amount in one order.');
  ...
  public function createOrderInvoiceApi($order, $inputs)
  {
    ...
    $this->newOrderInvoice($order->id, 'Service Charge', isset($inputs['service_charge']) ? $inputs['service_charge'] : 0);

    $this->newOrderInvoice($order->id, 'Service Tax', isset($inputs['service_tax']) ? $inputs['service_tax'] : 0);

    $this->newOrderInvoice($order->id, 'Donation Amount', isset($inputs['donation_amount']) ? $inputs['donation_amount'] : 0);

    $this->newOrderInvoice($order->id, 'Packaging Charge', isset($inputs['packaging_charge']) ? $inputs['packaging_charge'] : 0);
  ...
  public function getTotalOrderAmount($order,$inputs)
  {
    ...
    if($order->redeem_points==1)
    {
      ...
      if($user_points>($total * $max_percent / 100) && $total>0)
      {
        $used_points = ($total * $max_percent / 100);
      }
      else if($total>0)
      {
    ...
    $delivery_charge = $order->kitchen->delivery_charge ? $order->kitchen->delivery_charge :  $order->area->delivery_charge;
    ...
    $service_charge_amount = $total * $service_charge * 0.01;

    //Service Charge

    $this->newOrderInvoice($order->id,'Service Charge',$service_charge_amount);
    ...
    $total += $vat_amount+$service_tax_amount+$service_charge_amount+$packaging_charge+$order->donation_amount;
