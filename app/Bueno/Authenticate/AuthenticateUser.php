...
class  AuthenticateUser {
    ...
    public function execute($driver, $hasCode, $inputs)
    {
        ...
        $user = $this->userRepo->getUserByEmail($userData->email);

        if($user)
        {
          $this->loginUserByUserInstance($user);
        ...
        return redirect(route('users.socialregister.get'))->with('user', $userData);
    }

  /**
   * logs in the user by email only
   *
   * @param $userData
   * @throws ModelNotFoundException
   */
  public function loginUserByUserInstance($user)
  {
  ...
  public function loginUserByPhone($userData)
  {
    ...
    /**
     * retrieves the user data from valid token
     *
     * @param $driver
     * @return \Laravel\Socialite\Contracts\User
     */
    public function getSocialUserFromToken($driver, $token)
    {
        return Socialite::driver($driver)->userFromToken($token);
    }
