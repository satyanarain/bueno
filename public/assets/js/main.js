...
//promt user to download app (Andriod/ios), if user in msite
var checkifMsite = function(){
    if(!getCookie('msite')){
        var ua = navigator.userAgent;
        if (/iPhone|iPad|iPod/i.test(ua)){
           $('#ios_detect_modal').modal('show'); 
           setCookie('msite',true,1);
        }else if (/Android/i.test(ua)){
           $('#android_detect_modal').modal('show');
           setCookie('msite',true,1);
        }
    }
};
