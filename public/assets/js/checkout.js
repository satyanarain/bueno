    ...
    var updateTotalAmount = function(){
        ...
        maxCreditUsagePercent = parseFloat(credit_amount_elem.data('maxcreditusageercent'));
        
        if(points > (total * maxCreditUsagePercent /100))
            points = total * maxCreditUsagePercent /100;
        if(points > total/2){
        ...
        // 3. apply vat and tax
        vat = $('.vat_input').val() ? calculteTaxPercent(total, $('.vat_input').val()) : 0;
        service_tax = $('.service_tax_input').val() ? calculteTaxPercent(total, $('.service_tax_input').val()) : 0;
        service_charge = $('.service_charge_input').val() ? calculteTaxPercent(total, $('.service_charge_input').val()) : 0;
        value_added_tax = $('.value_added_tax_input').val() ? calculteTaxPercent(total, $('.value_added_tax_input').val()) : 0;
        merchandise = $('.merchandise_input').val() ? calculteTaxPercent(total, $('.merchandise_input').val()) : 0;
        
        total += parseFloat(vat) + parseFloat(service_tax) + parseFloat(service_charge) + parseFloat(value_added_tax);

        $('.vat_amount_text span').text(vat.toFixed(2));
        $('.service_tax_text span').text(service_tax.toFixed(2));
        $('.service_charge_text span').text(service_charge.toFixed(2));
        $('.value_added_tax_text span').text(value_added_tax.toFixed(2));
        
        //4. Delivery Charge
        delivery_charge = $('.delivery_charge_input').val() ? $('.delivery_charge_input').val() : 0;
        OrderDeliveryType = $(document).find('.user_order_delivery_type_row input[type=radio]:checked');
        if(parseInt(OrderDeliveryType.val())){
            $('.delivery_charge_amount').hide();
            delivery_charge = 0;
        }else{
            $('.delivery_charge_amount').show();
        }
        delivery_charge = parseFloat(delivery_charge);
        total += delivery_charge;
        $('.delivery_charge_text span').text(delivery_charge.toFixed(2));

        //Packaging Charge
        packaging_charge = $('.packaging_charge_input').val()  ? $('.packaging_charge_input').val() : 0;
        packaging_charge = parseFloat(packaging_charge);
        total += parseFloat(packaging_charge)
        $('.packaging_charge_text span').text(packaging_charge.toFixed(2));
    ...
    var checkIfOrderHasdeliveryType = function(){ alert("fdfgh");
        $('.user_address_checkout_error').remove();

        OrderDeliveryType = $(document).find('.user_order_delivery_type_row input[type=radio]:checked');

        if(OrderDeliveryType.val()){
            formValid = formValid && true;
        }else{
            formValid = formValid && false;
        }
    ...
    var checkIfUserHasAddresses = function(){
        ...
        if(parseInt(OrderDeliveryType.val())){
            formValid = formValid && true;
            $(".select_address_area").hide();
        }else{
            $(".select_address_area").show();
    ...
    var deliveryTypeChecked = function(){
        checkIfCartHasErrors();  
        updateTotalAmount();
    }
    ...
    var calculteTaxPercent = function(total, taxPercent){
        var tax_amount = total * taxPercent * .01;
        return tax_amount;
    };
    ...
    var addRemoveDonation = function(e){
        ...
        html += '<p class="total_content">₹ ' +  parseFloat(donationInput.data('amount')).toFixed(2) +' </p>';
        html += '</div>';
   ...
   var getTotalItemQuantitySum =  function () {
    ...
    var applyCoupon = function(e){
        ...
        if(!form[0].elements["payment_mode_id"].value){
            showCouponError('Please select a payment method');
            return false;
        }
        ...
        $.ajax({
            ...
            if(response.success){
                ...
                if(response.data.discount){
                    ...
                    html += '<p class="total_content">- ₹ '+ parseFloat(response.data.discount).toFixed(2) +'</p>';
                    html += '</div>';
    ...
    $(document).on('change', '.user_order_delivery_type_row input[type=radio]', deliveryTypeChecked);
